...
FRAME_CT = "e0f2ae2cf99177b0c87847f9b7cbb51bff6754ad"
FRAME_TAG = "146ff2a79d6fdd041276995b14c588e9"
PLAINTEXT = b'{"n":38,"s":0,"p":2}'
...
def test_envelope_matches_firmware():
    ...
    ciphertext, tag = crypto._xchacha_seal(
        crypto.derive_field_key(), nonce, bytes([38]), PLAINTEXT)
...
def test_public_aead_round_trip():
    ...
    nonce = bytes(range(24))
    ciphertext, tag = crypto.xchacha_seal(key, nonce, bytes([38]), PLAINTEXT)
    opened = crypto.xchacha_open(key, nonce, bytes([38]), ciphertext, tag)
